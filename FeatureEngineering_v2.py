...
def get_features(students, item_diff, max_stu, cluster, num_skills, datatype):
    ...
    data= pd.DataFrame({'student_id':stu_list, 'skill_id': p0_list,'skill_mastery': p1_list, 'ability_profile': p2_list, 'problem_difficulty': p3_list, 'correctness': p4_list, 'problems': p5_list})
    data.to_csv ("./v2/"+datatype+"_data.csv", index = None, header=True)
